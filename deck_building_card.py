...
def card_init(player_one, player_computer, central):
    """Init the players area and the central line, hand out hand card"""
    ...
    # hand out computer's hand cards
...
def print_card_information(central):
    """Print active line cards and supplement cards"""
...
def act_play_all(money, attack, player_one):
    """Player's action: play all the cards"""
...
def act_buy(money, player_one, central):
    """Player's action: buy some cards"""
    while money > 0:
        print("Available Cards")
        ind = 1
        for card in central['active']:
...
def act_attack(attack, player_computer):
    """Player's action: attack opponent"""
...
def judgement(player_one, player_computer, central):
    """Judge the result when played all the cards"""
...
if __name__ == '__main__':
    ...
    while game_continue:
        ...
        game_continue = game(player, computer, central_line)
        if not game_continue:
            play_game = raw_input('Do you want to play another game?(Y/N):')
            game_continue = (play_game=='Y')
            continue
